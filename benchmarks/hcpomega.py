...
import scienceplots
from .general import get_pathway_data,get_icsd_ref_energy,scatter_data,plot_data,unroll_pathway_metadata

def split_hcp_omega_path(data_collection:pd.DataFrame):
    '''
    split hcp_omega path to SSNEB relaxed and not SSNEB relaxed
    '''
    ssNEB=data_collection[data_collection['SSNEB']==True]
    nssNEB=data_collection[data_collection['SSNEB']==False]
...
def benchmark_hcp_omega_pathway(data_collection: pd.DataFrame):
    ...
    for key in keys:
        ...
        for pathway in pathways:
            pathway_static=static[static['pathway_number']==pathway]
            pathway_ssNEB=ssNEB[static['pathway_number']==pathway]
            images_static=pathway_static['value']
            images_ssNEB=pathway_ssNEB['value']
